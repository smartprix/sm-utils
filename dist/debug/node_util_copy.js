...
const internalUtil = require('./internal_util_copy');
...
if (typeof global.SIMD === 'object' && global.SIMD !== null) {
  ...
  const make = (extractLane, count) => {
    return (ctx, value, recurseTimes, visibleKeys, keys) => {
      ...
      for (let i = 0; i < count; i += 1) output[i] = formatPrimitive(ctx, extractLane(value, i));
      return output;
    };
  };
  ...
  if (typeof SIMD.Bool16x8 === 'function') simdFormatters.set(SIMD.Bool16x8, make(SIMD.Bool16x8.extractLane, 8));

  if (typeof SIMD.Bool32x4 === 'function') simdFormatters.set(SIMD.Bool32x4, make(SIMD.Bool32x4.extractLane, 4));

  if (typeof SIMD.Bool8x16 === 'function') simdFormatters.set(SIMD.Bool8x16, make(SIMD.Bool8x16.extractLane, 16));

  if (typeof SIMD.Float32x4 === 'function') simdFormatters.set(SIMD.Float32x4, make(SIMD.Float32x4.extractLane, 4));

  if (typeof SIMD.Int16x8 === 'function') simdFormatters.set(SIMD.Int16x8, make(SIMD.Int16x8.extractLane, 8));

  if (typeof SIMD.Int32x4 === 'function') simdFormatters.set(SIMD.Int32x4, make(SIMD.Int32x4.extractLane, 4));

  if (typeof SIMD.Int8x16 === 'function') simdFormatters.set(SIMD.Int8x16, make(SIMD.Int8x16.extractLane, 16));

  if (typeof SIMD.Uint16x8 === 'function') simdFormatters.set(SIMD.Uint16x8, make(SIMD.Uint16x8.extractLane, 8));

  if (typeof SIMD.Uint32x4 === 'function') simdFormatters.set(SIMD.Uint32x4, make(SIMD.Uint32x4.extractLane, 4));

  if (typeof SIMD.Uint8x16 === 'function') simdFormatters.set(SIMD.Uint8x16, make(SIMD.Uint8x16.extractLane, 16));
}
...
function inspectPromise(p) {
  ...
  if (!binding.isPromise(p)) return null;
  const mirror = Debug.MakeMirror(p, true);
  return { status: mirror.status(), value: mirror.promiseValue().value_ };
}

function formatValue(ctx, value, recurseTimes) {
  if (ctx.showProxy && (typeof value === 'object' && value !== null || typeof value === 'function')) {
    var proxy = undefined;
    var proxyCache = ctx.proxyCache;
    if (!proxyCache) proxyCache = ctx.proxyCache = new Map();
  ...
  if (Array.isArray(value)) {
    ...
    if (constructor && constructor.name === 'Array') constructor = null;
    braces = ['[', ']'];
  ...
  } else if (simdFormatters && typeof value.constructor === 'function' && (formatter = simdFormatters.get(value.constructor))) {
    braces = ['[', ']'];
  } else {
    ...
    if (promiseInternals) {
      ...
      if (binding.isMapIterator(value)) {
        ...
        if (constructor && constructor.name === 'Object') constructor = null;
        braces = ['{', '}'];
...
function formatPrimitive(ctx, value) {
  ...
  if (type === 'number') return formatNumber(ctx, value);
...
function reduceToSingleString(output, base, braces, breakLength) {
  var length = output.reduce(function (prev, cur) {
    return prev + cur.replace(/\u001b\[\d\d?(\;\d\d?)?m/g, '').length + 1;
  }, 0);
