...
function errorFrameToString(frame, background = 'magenta', foreground = 'white') {
	...
	if (!fileName) {
		return chalk.bgYellow(' ' + chalk.black.bold(_.repeat('-', process.stdout.columns - 2)));
	}
	...
	return chalk[`bg${_.upperFirst(background)}`](' ' + chalk[foreground](chalk.bold(fileName) + ' at ' + chalk.bold(lineNumber) + ':' + columnNumber + ' in ' + chalk.bold(functionName) + headerSpace));
}
